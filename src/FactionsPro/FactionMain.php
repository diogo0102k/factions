...
class FactionMain extends PluginBase implements Listener {
    ...
    public function sendListOfTop10FactionsTo($s) {
        ...
        $row = array();
        $i = 0;
        $s->sendMessage($this->formatMessage("§l§b»§r§e-=§f[§aTop 10 Leading Guildz§f]§e=-§l§b«", true));
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            $j = $i + 1;
            $cf = $resultArr['faction'];
            $pf = $this->getFactionPower($cf);
            $df = $this->getNumberOfPlayers($cf);
            $s->sendMessage(TextFormat::BLUE . TextFormat::GOLD . "§l§b» §r§e$j §f:  " . TextFormat::GREEN . "$cf" . TextFormat::GOLD . " with " . TextFormat::RED . "$pf GuildsPowerz" . TextFormat::GOLD . " and " . TextFormat::LIGHT_PURPLE . "have $df PLAYERS" . TextFormat::RESET);
            $i = $i + 1;
        }
    }
        ...
        if ($this->cornerIsInPlot($x + $arm, $z + $arm, $x - $arm, $z - $arm)) {
            ...
            if ($this->prefs->get("EnableOverClaim")) {
                if ($power_sender < $power_claimedBy) {
                    $sender->sendMessage($this->formatMessage("Essa Área Já está Claimada Por $claimedBy com $power_claimedBy STR. Tua Guilda tem $power_sender poder. Não possuis poder suficiente para overclaim ."));
                } else {
                    $sender->sendMessage($this->formatMessage("Esta Área Já Está Claimada por $claimedBy com $power_claimedBy STR. Tua Guilda tem $power_sender poder. digite /f overclaim para dar overclaim na guilda $claimedBy"));
                }
                return false;
            } else {
                $sender->sendMessage($this->formatMessage("Overclaim está desactivado."));
                return false;
            }
        }
